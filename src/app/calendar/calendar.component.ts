...
export class CalendarComponent implements AfterViewInit {
  ...
  constructor(private ds: DataService, private zone: NgZone) {
    console.log('is build for BC: ' + GlobalConstants.isBuildForBusinessCentral);
    console.log('setting up calendar');
    this.viewDay();

    //Listen for add_new_event event
    window.addEventListener('add_new_event', (event: any) => {
      this.zone.run(() => {
        console.log(event['detail'].args[0]);
        this.addEvent(event['detail'].args[0].text, event['detail'].args[0].start, event['detail'].args[0].end, event['detail'].args[0].backColor);
      });
    });

    if(GlobalConstants.isBuildForBusinessCentral) {
      // @ts-ignore
      Microsoft.Dynamics.NAV.InvokeExtensibilityMethod('EventRanFromAngular', ["test", new Date()])
      console.log('invoked');
    }

    // console.log(GlobalConstants.isBuidForBusinessCentral);
  }
  ...
  addEvent(text: string, start: DayPilot.Date, end: DayPilot.Date, backColor: string): void {
    this.ds.events.push(
      {
        id: this.ds.events.length + 1,
        text: text,
        backColor: backColor,
        start: start,
        end: end,
      ...
      args.data.areas = [
        {
          ...
          width: 20,
          height: 20,
          // symbol: "//hellebrekers.000webhostapp.com/agendaapp/images/daypilot.svg#minichevron-down-2",
          fontColor: "#fff",
        ...
        },
        {
          ...
          width: 20,
          height: 20,
          // symbol: "//hellebrekers.000webhostapp.com/agendaapp/images/daypilot.svg#x-circle",
          fontColor: "#fff",
          action: "None",
